...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, gamma, x_normalized, sample_mean, sample_var, eps)
        pass
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # Unpack cache
    x, gamma, x_norm, mean, var, eps = cache
    N, D = x.shape
    
    # Gradient for beta (just sum over batch dimension)
    dbeta = np.sum(dout, axis=0)
    
    # Gradient for gamma
    dgamma = np.sum(dout * x_norm, axis=0)
    
    # Gradient for x (this is the complicated part)
    # We need to backprop through the normalization operation
    ...
    dmean1 = np.sum(dx_norm * -1 / std, axis=0)
...
def layernorm_backward(dout, cache):
    ...
    dx_hat=dout*gamma
    dx_minus_mean=x-sample_mean
    inv_std=1.0/np.sqrt(sample_var+eps)
    dvar=np.sum(dx_hat*dx_minus_mean*-0.5*(inv_std**3),axis=1,keepdims=True)
    dmean=np.sum(dx_hat*-inv_std,axis=1,keepdims=True)/D
    dx=(dx_hat*inv_std)+(dvar*2.0*dx_minus_mean/D)+(dmean/D)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache=(x_reshaped,x_hat,gamma,beta,G,mean,var,eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    x_reshaped,x_hat,gamma,beta,G,mean,var,eps=cache
    N,C,H,W=x_hat.shape
    ...
    x_hat_reshaped=(x_reshaped-mean)*inv_std
    
    D_group=(C//G)*H*W
    
    dvar=np.sum(dx_hat_reshaped*(x_reshaped-mean)*-0.5*(inv_std**3),axis=(2,3,4),keepdims=True)

    dmean=np.sum(dx_hat_reshaped*-inv_std,axis=(2,3,4),keepdims=True)+\
          dvar*np.sum(-2.0*(x_reshaped-mean),axis=(2,3,4),keepdims=True)/D_group
    dx_reshaped=(dx_hat_reshaped*inv_std)+\
                (dvar*2.0*(x_reshaped-mean)/D_group)+\
                (dmean/D_group)
